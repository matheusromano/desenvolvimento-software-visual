...
namespace Lista3
{
	...
	class Aeroporto
	{
		...
		{
			//Pega o voo pelo horario passado
			var voo = GetVooPorHorario(horario);

			//seleciona os ids da reserva e compora com os ids do voo selecionado e transforma em uma lista 
			var reservas = Reservas.Where(r => r.IdVoo == voo.Id).ToList();

			// Loop sobre a lista de reservas por reserva
			foreach(var reserva in reservas)
			{
				// Remove da lista de reservas a reserva encontrada
				Reservas.Remove(reserva);
			}
			// Remove o voo determinado na linha 185
			Voos.Remove(voo);

			// Novamente loop sobre a lista de reservas agora limpa das reservas e voos cancelados 
			foreach(var reserva in reservas)
			{
				// Pega o id do passageiro na reserva indicada
				var passageiro = GetPassageiroPorId(reserva.IdPassageiro);

				// Define um proximo voo para o passageiro se o local de destino for o mesmo e o numero de acentos ocupados for menor que que o numero total de acentos e organiza pelo horario da partida
				var proximoVoo = Voos.Where(v => v.LocalDestino == voo.LocalDestino && v.AssentosOcupados < v.TotalAssentos).OrderBy(v => v.HorarioPartida).FirstOrDefault();

				// Testa se nao foi encontrado um voo que bate com as condicoes
				if(proximoVoo == null)
				{
					// lanca uma excecao caso seja nula a variavel
					throw new Exception("nao foi possivel reagendar o voo de " + passageiro.Nome + " para o destino " + voo.LocalDestino);
				}

				// Incremeta o numero de acessentos ocupados 
				proximoVoo.AssentosOcupados++;

				// Gera uma nova reserva com os dados do passageiro e do novo voo que foi encontrado
				var novaReserva = new Reserva(GetUniqueId(), passageiro.Id, proximoVoo.Id);

				// Adiciona a nova reserva a lista de reservas
